...
const ServerPerformancePage = () => {
  ...
  const handleRefresh = useCallback(
    () =>
      queryClient.resetQueries({ queryKey: ['admin', 'serverPerformance'] }),
    [queryClient]
  );
      ...
      <div className="space-y-6 px-4 py-8">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <StatCard
            label="CPU"
            value={
              sys?.cpu?.usagePercent != null
                ? `${sys.cpu.usagePercent.toFixed(0)}%`
                : 'N/A'
            }
            icon={Cpu}
            ...
            color={
              (sys?.cpu?.usagePercent ?? 0) > 80
                ? 'text-red-600'
                : 'text-foreground'
            }
            subtitle={
              sys?.cpu?.count != null ? `${sys.cpu.count} cores` : undefined
            }
            isLoading={isLoading}
          />
          <StatCard
            label="Memory"
            value={
              sys?.memory?.usagePercent != null
                ? `${sys.memory.usagePercent.toFixed(0)}%`
                : 'N/A'
            }
            icon={MemoryStick}
            iconColor="text-emerald-600"
          ...
          <StatCard
            label="Storage"
            value={
              sys?.storage?.usagePercent != null
                ? `${sys.storage.usagePercent.toFixed(0)}%`
                : 'N/A'
            }
            icon={HardDrive}
          ...
          <>
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-medium">
                    <AnimatedNumber
                      value={(sys?.memory?.usedMB ?? 0) / 1024}
                      decimals={2}
                      suffix=" GB"
                    />
                  </span>
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium">
                      <AnimatedNumber
                        value={(sys?.memory?.totalMB ?? 0) / 1024}
                        decimals={2}
                        suffix=" GB"
                      />
                    </p>
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      <AnimatedNumber
                        value={(sys?.memory?.freeMB ?? 0) / 1024}
                        decimals={2}
                        suffix=" GB"
                      />
                    </p>
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      <AnimatedNumber
                        value={(sys?.memory?.availableMB ?? 0) / 1024}
                        decimals={2}
                        suffix=" GB"
                      />
                    </p>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {avail?.healthyServices}/{avail?.totalServices} services
                  healthy — {(avail?.uptimePercent ?? 0).toFixed(0)}%
                  availability
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid gap-4 sm:grid-cols-2">
                  {d?.services &&
                    Object.entries(d.services).map(([name, svc]) => {
                      ...
                      return (
                        <Card
                          key={name}
                          className={`p-4 border-l-4 ${healthy ? 'border-l-green-500' : 'border-l-red-500'}`}
                        >
