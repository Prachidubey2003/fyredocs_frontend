...
export const fetchBusiness = (days = 30) =>
  unwrap(
    apiRequest<ApiResponse<BusinessData>>(
      `/admin/metrics/business?days=${days}`
    )
  );
...
export const fetchEngagement = (days = 30) =>
  unwrap(
    apiRequest<ApiResponse<EngagementData>>(
      `/admin/metrics/engagement?days=${days}`
    )
  );
...
export const fetchReliability = (days = 30) =>
  unwrap(
    apiRequest<ApiResponse<ReliabilityData>>(
      `/admin/metrics/reliability?days=${days}`
    )
  );
...
export const fetchServerPerformance = () =>
  unwrap(
    apiRequest<ApiResponse<ServerPerformanceData>>(
...
export const fetchApiPerformance = async (
  params?: EndpointQueryParams
): Promise<{ data: ApiPerformanceData; meta?: PaginationMeta }> => {
